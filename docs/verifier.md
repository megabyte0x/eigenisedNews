...
| `research_outputs` | Yes | For `/research`, verifies response article/prompt/run fields and output hashes, including the main-agent `mainSummary`, against the signed research manifest. |
| `research_raw` | Yes, if `raw` is present | For `/research`, verifies exact planner/pro/contra/summary prompts and raw outputs, and checks the planner output reproduces `proPrompt`/`contraPrompt`. |
...
The research verifier branch checks the manifest hash, signer recovery, article refetch drift when `--refetch` is enabled, response/output hashes, raw planner/pro/contra/summary outputs when present, and EigenCompute provenance when configured.
